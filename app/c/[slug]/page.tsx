...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  // If a sub-filter is active, narrow the grid to just that sub.
  // Otherwise show whatever the category endpoint returned (the whole
  // subtree for a top-level category, or just the category's products
  // for a leaf).
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
        <div>
          <div className="text-xs text-neutral-500">
            ...
            {selectedSub && children?.find((c) => c.slug === selectedSub) && (
              <>
                {" / "}
                {children.find((c) => c.slug === selectedSub)?.name}
              </>
            )}
          </div>
      ...
      {products.length === 0 ? (
        <div className="text-center py-24 text-neutral-400">
          ...
          <p className="text-sm mt-2">
            {selectedSub ? "Try clearing this filter or pick a different category." : "Check back soon — new drops arrive weekly."}
          </p>
          <Link
            href={baseHref}
...
function PillLink({ label, href, active }: { label: string; href: string; active: boolean }) {
  return (
    <Link
      href={href}
